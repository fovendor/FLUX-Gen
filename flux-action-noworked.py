...
class Action:
    class Valves(BaseModel):
        pipeline_url: str = "http://0.0.0.0:9099"
        pipeline_auth: str = "0p3n-w3bu!"

    def __init__(self):
        self.valves = self.Valves()

    async def _send_to_pipeline(self, data: dict) -> dict:
        """Отправляет данные в pipeline и возвращает результат"""
        try:
            headers = {
                "Content-Type": "application/json",
                "Authorization": f"Bearer {self.valves.pipeline_auth}",
            }
            response = requests.post(
                f"{self.valves.pipeline_url}/v1/generate",
                headers=headers,
                json={"model_id": "flux-pro-fill", "inpainting_data": data},
            )
    ...
    async def action(
        ...
        # Проверяем, получены ли данные для inpainting
        ...
        if inpainting_data:
                ...
                await __event_emitter__(
                    {
                        "type": "status",
                        "data": {
                            "description": "Отправляем запрос на обработку изображения...",
                            "done": False,
                        },
                    }
                )
            ...
            result = await self._send_to_pipeline(inpainting_data)
                    ...
                    await __event_emitter__(
                        {
                            "type": "status",
                            "data": {
                                "description": f"Ошибка: {result.get('message')}",
                                "done": True,
                            },
                        }
                    )
            ...
            return {"status": "ok", "message": "Запрос на обработку отправлен"}

        # Стандартная логика создания HTML-интерфейса
...
"""
        ...
        if __event_emitter__:
            ...
            if not existing_artifact_msg:
                ...
                old_msg_id = existing_artifact_msg.get("id")
                if old_msg_id:
                    await __event_emitter__(
                        {
                            "type": "edit_message",
                    ...
                    await __event_emitter__(
                        {
                            "type": "status",
                            "data": {
                                "description": "Артефакт уже существует, откройте его в меню чата",
                                "done": True,
                            },
                        }
                    )
