...
class Action:
    ...
    async def action(
        ...
        artifact_html = f"""
```html
<head><meta charset="UTF-8"><title>Inpainting Helper</title>
<style>body{{font-family:Arial,sans-serif;margin:20px;max-width:1000px;}}canvas{{max-width:100%;height:auto;border:1px solid #ccc;margin:10px 0;}}.input-group{{margin:10px 0;}}textarea{{width:100%;height:150px;margin:10px 0;resize:vertical;}}button{{padding:8px 16px;margin:5px;cursor:pointer;background:#007bff;color:white;border:none;border-radius:4px;}}button:hover{{background:#0056b3;}}.coords-info{{margin:10px 0;color:#666;}}</style>
</head>
<body>
<h1>Inpainting Helper</h1>
<p>Изображение загружается фоновым fetch'ом (без огромного base64 в чате).</p>
<div class="coords-info" id="coordsInfo">Не выделено</div>
<canvas id="imageCanvas"></canvas>
<div>
  <button id="generateMaskBtn">Сформировать маску</button>
  <button id="resetBtn">Сброс</button>
</div>
<div class="input-group">
  <label for="promptInput">Введите промт для замены:</label>
  <textarea id="promptInput" placeholder="Опишите, что вы хотите видеть в выделенной области..."></textarea>
</div>
<div>
  <h2>Base64 исходного изображения</h2>
  <textarea id="originalBase64" readonly></textarea>
  <button id="copyOriginalBtn">Копировать</button>
</div>
<div>
  <h2>Base64 маски (PNG)</h2>
  <textarea id="maskBase64" readonly></textarea>
  <button id="copyMaskBtn">Копировать</button>
</div>
<script>
...
function updateCoordsInfo(){{
  const info = currentRect
    ? 'X:' + Math.round(currentRect.x)
      + ',Y:' + Math.round(currentRect.y)
      + ',W:' + Math.round(currentRect.w)
      + ',H:' + Math.round(currentRect.h)
    : 'Не выделено';
  document.getElementById('coordsInfo').textContent = info;
}}

document.getElementById('generateMaskBtn').addEventListener('click', function(){{
  if(!currentRect || !originalImage){{
    alert('Пожалуйста, выделите область на изображении.');
    return;
  }}
  ...
  maskCtx.fillRect(currentRect.x, currentRect.y, currentRect.w, currentRect.h);

  const maskBase64 = maskCanvas.toDataURL('image/png');
  document.getElementById('maskBase64').value = maskBase64;
}});

document.getElementById('copyOriginalBtn').addEventListener('click', ()=>{{copyToClipboard('originalBase64');}});
document.getElementById('copyMaskBtn').addEventListener('click', ()=>{{copyToClipboard('maskBase64');}});
...
document.getElementById('resetBtn').addEventListener('click', function(){{
  ...
  originalImage=null;
  currentRect=null;
  document.getElementById('originalBase64').value='';
  document.getElementById('maskBase64').value='';
  document.getElementById('coordsInfo').textContent='Не выделено';
  document.getElementById('promptInput').value='';
  canvas.width=0;canvas.height=0;
}});

// Подгружаем файл из /cache/image/generations/{filename} (без вставки base64 в чате):
...
fetch(fileUrl)
  ...
  .then(blob => {{
    // Конвертим blob -> dataURL
    const reader = new FileReader();
    reader.onload = e => {{
      const base64Data = e.target.result;
      // Показываем base64 в textarea (при желании)
      document.getElementById('originalBase64').value = base64Data;
  ...
  .catch(err=>{{alert('Fetch error: '+ err); console.error(err);}});
</script>
</body>
```
""".strip()
